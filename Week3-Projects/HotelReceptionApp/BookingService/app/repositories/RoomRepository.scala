...
class RoomRepository @Inject()(dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext) {
  ...
  def getAvailableRoomIdByRoomNo(roomNo: Int): Future[Option[Int]] = db.run {
    rooms.filter(room => {room.roomNo === roomNo && room.roomStatus=== "AVAILABLE"}).map(_.roomID).result.headOption
  }
