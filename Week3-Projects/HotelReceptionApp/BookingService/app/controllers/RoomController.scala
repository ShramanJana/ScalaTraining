...
class RoomController @Inject()(val controllerComponents: ControllerComponents, roomStatusUpdateTask: RoomStatusUpdateTask, roomRepository: RoomRepository,
  ...
  case class GuestData(name: String, email: String, address: String, idProof: String, guestStatus: String)

  case class GuestAllocationRequest(roomNo: Int, guests: Seq[GuestData], endDate: LocalDate)
    ...
    request.body.validate[GuestAllocationRequest].fold(
      ...
      allocationRequest => {
        if (allocationRequest.guests.size > 3) {
          ...
          roomRepository.getAvailableRoomIdByRoomNo(allocationRequest.roomNo).flatMap {
            case Some(roomId) =>
